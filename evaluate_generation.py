...
def make_predictions(
    src_dataset: datasets.Dataset,
    path_model: Path,
    src_col: str = "source_text",
    target_qtypes: list = None,
    add_src_prefix: bool = None,
    batch_size: int = 8,
):
    """
    ...
    def seq2seq_batch(batch_examples):
        input_ids = tokenizer(
            batch_examples, padding=True, truncation=True, return_tensors="pt"
        )["input_ids"]
        if torch.cuda.is_available():
    ...
    if target_qtypes:
        # TODO: make this part process in batches
        if add_src_prefix:
            rt = src_dataset.map(
                lambda e: {qt: seq2seq(e[src_col], f"{qt}: ") for qt in target_qtypes}
            )
        else:
            rt = src_dataset.map(
                lambda e: {qt: seq2seq(e[src_col]) for qt in target_qtypes}
            )
    # Otherwise, make one prediction and store the result in 'yhat'
    else:
        rt = src_dataset.map(
            lambda e: {"yhat": seq2seq_batch(e[src_col])},
            batched=True,
...
def run_predict_valid_set(**kargs):
    ...
    p_args = utils.process_args(
        kargs["base_model_name"], kargs["tokenizer_args"], kargs["question_labels"]
    )
    ...
    if not path_valid_dataset.exists():
        raise FileNotFoundError(
            f"validation dataset does not exist: {path_valid_dataset}"
        )
...
def evaluate_translation(
    ...
    for qt in target_qtypes:
        ...
        def process_example(e):
            ...
            return {
                "_reference": ref,
                "_reference_tokens": ref_tokens,
                "_prediction": pred,
                "_prediction_tokens": pred_tokens,
            }
        ...
        logger.info("Compute the scores...")
        references_bleu = [[i] for i in references_tokens]
        bleu1 = metric_bleu.compute(
            predictions=predictions_tokens, references=references_bleu, max_order=1
        )["bleu"]
        bleu2 = metric_bleu.compute(
            predictions=predictions_tokens, references=references_bleu, max_order=2
        )["bleu"]
        bleu3 = metric_bleu.compute(
            predictions=predictions_tokens, references=references_bleu, max_order=3
        )["bleu"]
        bleu4 = metric_bleu.compute(
            predictions=predictions_tokens, references=references_bleu, max_order=4
        )["bleu"]
        ...
        performance.append(
            {
                "qtype": qt,
                "num_questions": len(gen_items_qt),
                "bleu1": bleu1,
                "bleu2": bleu2,
                "bleu3": bleu3,
                "bleu4": bleu4,
...
def run_evaluate_translation(**kargs):
    ...
    if test_on_squad:
        ...
        if not path_gen_questions.exists():
            raise FileNotFoundError(
                f"validation dataset does not exist: {path_gen_questions}"
            )
        # Load the validation set
